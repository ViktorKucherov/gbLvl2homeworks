...
public class Person implements Participant{
    ...
    public Person(String n, double j, int d) {
        this.name = n;
        this.jumpH = j;
        this.distanceL = d;
    }
    @Override
    public double jump() {
        System.out.println("Спортсмен " + this.name + " прыгает на " + this.jumpH + " метров.");
        return jumpH;
    }

    @Override
    public int run() {
        System.out.println("Спортсмен " + this.name + " пробегает дистанцию в " + this.distanceL + " метров.");
        return distanceL;
    }
